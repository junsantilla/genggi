...
export default async function Profile({
  ...
  // TODO: To fix the force no display feature
  // This will force to show the user information if it is publicly available
  const customCss = customCssEnabled
    ? `${user.theme?.customCss?.trim()} body, body > *, #wrap, #wrap *, .profile-content, .profile-content * { display: revert !important;}`
    : undefined;
  let safeCustomCss = customCss?.replace(/<\/style/gi, "<\\/style");

  // TODO: To get all the animation duration and set minimum animation
  // This will solve the problem for flickering display
  const minDuration = 15

  safeCustomCss = safeCustomCss?.replace(
    /animation\s*:[^;]*?\b(\d+(?:\.\d+)?)s\b/gmi,
    (match, duration) => {
      const time = Number(duration)

      if (time < minDuration) {
        return match.replace(`${duration}s`, `${minDuration}s`)
      }
