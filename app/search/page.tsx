...
export default async function SearchPage({
  ...
  searchParams: Promise<{ q?: string; gender?: string; location?: string; interests?: string; status?: string }>;
}) {
  ...
  const results = await db.collection("users").find(filter).sort({ lastActive: -1 }).limit(50).toArray();

  const statuses = await Promise.all(
    results.map((r) => getFriendshipStatus(user._id.toString(), r._id.toString()))
  );
  ...
  return (
    <div className="max-w-[760px] w-full mx-auto bg-white border border-[#6699cc] sm:border-x">
      ...
      <div className="p-4 flex flex-col gap-4">
        <Box title="Filters">
          <form action="/search" method="get" className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
            <input name="q" defaultValue={q} placeholder="Name or username" className="input" />
            <select name="gender" defaultValue={gender} className="input">
              {GENDERS.map((g) => (
                <option key={g || "any"} value={g}>{g ? g : "Any gender"}</option>
              ))}
            </select>
            <input name="location" defaultValue={location} placeholder="Location (e.g. San Jose)" className="input" />
            ...
            <input name="interests" defaultValue={interests} placeholder="Interest (e.g. LAN parties)" className="input" />
