...
export default async function AdminPage({
    ...
    const bulletinSetting = await db
        .collection("settings")
        .findOne({ key: "bulletin" });
    const bulletinPostLimit =
        (bulletinSetting?.maxBulletinPostsPerHour as
            | number
            | null
            | undefined) ?? null;
    ...
    const reports = await db
        ...
        .toArray();
    const reportedIds = [
        ...new Set(reports.map((r) => r.reportedId.toString())),
    ];
    const reportedUsers =
        reportedIds.length > 0
            ? await db
                  .collection("users")
                  .find({
                      _id: { $in: reportedIds.map((id) => new ObjectId(id)) },
                  })
                  .toArray()
            : [];
    ...
    const testimonials = await db
        ...
        .toArray();
    const testiIds = [
        ...new Set(testimonials.map((t) => t.profileId.toString())),
    ];
    const testiProfiles =
        testiIds.length > 0
            ? await db
                  .collection("users")
                  .find({
                      _id: { $in: testiIds.map((id) => new ObjectId(id)) },
                  })
                  .toArray()
            : [];
                ...
                <Box title="Site Statistics">
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {stats.map(([label, n]) => (
                            <div
                                key={label}
                                className="border border-[#6699cc] p-2 text-center bg-[#f5f9ff]"
                            >
                                <div className="text-xl font-bold text-[#2c4d80]">
                                    {n}
                                </div>
                                ...
                                >
                                    <div className="flex items-center justify-between gap-2 flex-wrap">
                                        ...
                                        <div className="flex gap-1.5">
                                            <ActionButton
                                                action={adminReviewReportAction.bind(
                                                    null,
                                                    r._id.toString(),
                                                    "resolved",
                                                )}
                        ...
                        users.map((u) => (
                            ...
                            >
                                <div className="text-[12px]">
                                    ...
                                    <span
                                        className={`ml-1 px-1 text-[10px] font-bold ${u.role === "admin" ? "bg-[#cc3399] text-white" : "bg-[#dbe9f7] text-[#2c4d80]"}`}
                                    >
                                        {u.role}
                                    </span>
                                    {u.banned && (
                                        <span className="ml-1 px-1 text-[10px] font-bold bg-red-600 text-white">
                                            BANNED
                                        </span>
                                    )}
                                ...
                                <div className="flex gap-1.5 flex-wrap">
                                    <ActionButton
                                        action={adminSetBannedAction.bind(
                                            null,
                                            u._id.toString(),
                                            !u.banned,
                                        )}
                                        className={
                                            u.banned ? "btn" : "btn btn-danger"
                                        }
                                    >
                                        {u.banned ? "Unban" : "Ban"}
                                    </ActionButton>
                                    <ActionButton
                                        action={adminSetRoleAction.bind(
                                            null,
                                            u._id.toString(),
                                            u.role === "admin"
                                                ? "user"
                                    ...
                                    <ActionButton
                                        action={adminDeleteUserAction.bind(
                                            null,
                                            u._id.toString(),
                                        )}
                                        className="btn btn-danger"
