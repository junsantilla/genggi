    ...
    const counterparts =
        counterpartIds.length > 0
            ? await db
                  ...
                  .find({
                      _id: {
                          $in: counterpartIds.map((id) => new ObjectId(id)),
                      },
                  })
                  .toArray()
            : [];
    const counterpartMap = new Map(
        counterparts.map((c) => [c._id.toString(), c]),
    );
    const nameOf = (id: string) =>
        counterpartMap.get(id)?.displayName || "Unknown";
    const photoOf = (id: string) =>
        counterpartMap.get(id)?.photo as string | undefined;
        ...
        <div className="max-w-[960px] w-full mx-auto bg-white border border-[#6699cc] sm:border-x">
            ...
            <div className="p-4 flex flex-col gap-4">
                <Box
                    title={
                        composeTarget
                            ? `Compose to ${composeTarget.displayName}`
                            : "Compose"
                    }
                >
                    {composeTarget ? (
                ...
                <Box title={`Inbox (${threads.size} conversations)`}>
                    {threads.size === 0 ? (
                        ...
                        [...threads.entries()].map(
                            ([otherId, { messages: msgs }]) => {
                                const unread = msgs.filter(
                                    (m) =>
                                        m.recipientId.toString() === uid &&
                                        !m.read,
                                ).length;
                                const last = msgs[0];
                                const lastFromMe =
                                    last.senderId.toString() === uid;
                                return (
                                    <Link
                                        key={otherId}
