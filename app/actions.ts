...
export async function signupAction(
    ...
    if (
        ...
        mathSecond < 1 ||
        mathSecond > 9 ||
        mathAnswer !== mathFirst + mathSecond
    )
...
export async function updateProfileAction(
    ...
    patch.whoCanFriendRequest = String(
...
export async function updateThemeAction(
    ...
    await getDb()
        .collection("users")
        .updateOne({ _id: user._id }, { $set: { theme } });
    revalidatePath(`/${user.username}`);
    revalidatePath("/edit");
    return { ok: true };
}
            ...
            { _id: user._id },
            {
                $set: {
                    ...
                    whoCanMessage: String(
                        formData.get("whoCanMessage") || "everyone",
                    ),
...
export async function sendFriendRequestAction(
    ...
    if (await isBlocked(targetId, user._id.toString()))
...
export async function sendMessageAction(
    ...
    const canMessage =
        ...
        (recipient.whoCanMessage === "friends" &&
            (await areFriends(user._id.toString(), recipientId)));
    if (!canMessage) return { error: "This user is not accepting messages." };
    if (await isBlocked(recipientId, user._id.toString()))
        return { error: "You are blocked by this user." };
...
export async function markMessageReadAction(
    ...
    await getDb()
        .collection("messages")
        .updateOne(
            { _id: new ObjectId(messageId), recipientId: user._id },
            { $set: { read: true } },
        );
    revalidatePath("/messages");
    return { ok: true };
...
export async function writeTestimonialAction(
    ...
    if (await isBlocked(profileId, user._id.toString()))
        return { error: "You are blocked by this user." };
...
export async function approveTestimonialAction(
    ...
    await db
        .collection("testimonials")
        .updateOne({ _id: t._id }, { $set: { status: "approved" } });
    revalidatePath(`/${user.username}`);
    return { ok: true };
}
...
export async function deleteTestimonialAction(
    ...
    const t = await db
        .collection("testimonials")
        .findOne({ _id: new ObjectId(testimonialId) });
    if (!t) return { error: "Testimonial not found." };
...
export async function getMoreMembersAction(
    cursor: MembersCursor,
): Promise<Awaited<ReturnType<typeof getMembersPage>>> {
...
export async function deleteGroupAction(
    ...
    const posts = await db
        ...
        .toArray();
    if (group.photoPublicId)
        await destroyImage(group.photoPublicId).catch(() => {});
    await Promise.all(
        posts.map((post) =>
            post.photoPublicId
                ? destroyImage(String(post.photoPublicId)).catch(() => {})
                : Promise.resolve(),
        ),
    );
    await db
        .collection("groups")
        .deleteOne({ _id: group._id, ownerId: user._id });
    await Promise.all([
...
export async function requestGroupJoinAction(
    groupId: string,
): Promise<ActionResult> {
    const user = await requireUser();
    const group = await getGroupById(groupId);
    ...
    if (existing?.status === "pending")
        return { error: "Your join request is pending." };
    await db
        .collection("groupMembers")
        .insertOne({
            groupId: group._id,
...
export async function reviewGroupJoinAction(
    groupId: string,
    memberId: string,
    approve: boolean,
): Promise<ActionResult> {
...
export async function reactToGroupPostAction(
    groupId: string,
    postId: string,
    type: string,
): Promise<ActionResult> {
    ...
    if (existing?.type === type)
        await db.collection("groupReactions").deleteOne({ _id: existing._id });
    else if (existing)
        await db
            .collection("groupReactions")
...
export async function updateGroupCommentAction(
    groupId: string,
    commentId: string,
    formData: FormData,
): Promise<ActionResult> {
...
export async function deleteGroupCommentAction(
    groupId: string,
    commentId: string,
): Promise<ActionResult> {
    const user = await requireUser();
...
export async function createGroupCommentAction(
    groupId: string,
    postId: string,
    formData: FormData,
): Promise<ActionResult> {
...
export async function createGroupPostAction(
    ...
    const group = await getGroupById(groupId);
    if (!group || !(await canAccessGroup(group, user._id.toString())))
        return { error: "You must be an approved member to post." };
    const body = String(formData.get("body") || "").trim();
    const file = formData.get("photo");
...
const BULLETIN_VISIBILITIES: BulletinVisibility[] = [
    "public",
    "friends",
    "private",
];
...
export async function createBulletinPostAction(
    ...
    if (!BULLETIN_VISIBILITIES.includes(visibilityValue))
        return { error: "Invalid post visibility." };

    // Enforce the admin-configured per-hour posting limit.
    const bulletinSetting = await getDb()
        .collection("settings")
        .findOne({ key: "bulletin" });
    const postsPerHour = bulletinSetting?.maxBulletinPostsPerHour as
        | number
        | null
        | undefined;
...
export async function deleteBulletinPostAction(
    postId: string,
): Promise<ActionResult> {
    const user = await requireUser();
    const db = getDb();
...
export async function deleteBulletinCommentAction(
    commentId: string,
): Promise<ActionResult> {
    const user = await requireUser();
    const db = getDb();
    ...
    await db
        ...
        .updateOne(
            { blockerId: user._id, blockedId: new ObjectId(targetId) },
            {
                $setOnInsert: {
                    blockerId: user._id,
                    blockedId: new ObjectId(targetId),
                    createdAt: new Date(),
                },
            },
            { upsert: true },
...
export async function reportUserAction(
    ...
    const user = await requireUser();
    const reason =
        String(formData.get("reason") || "").trim() || "Reported user";
    await getDb()
        .collection("reports")
        .insertOne({
            reporterId: user._id,
...
export async function markNotificationReadAction(
    ...
    await db
        ...
        .updateOne(
            { _id: user._id },
            { $set: { notificationAcknowledgedAt: acknowledgedAt } },
        );
...
export async function adminSetBannedAction(
    targetId: string,
    banned: boolean,
): Promise<ActionResult> {
    await requireAdmin();
    await getDb()
        .collection("users")
        .updateOne({ _id: new ObjectId(targetId) }, { $set: { banned } });
    revalidatePath("/admin");
    return { ok: true };
}
...
export async function adminSetRoleAction(
    ...
    await getDb()
        .collection("users")
        .updateOne({ _id: new ObjectId(targetId) }, { $set: { role } });
    revalidatePath("/admin");
    return { ok: true };
}
...
async function deleteAllUserData(db: Db, oid: ObjectId): Promise<void> {
    ...
    await Promise.all([
        ...userPosts
            ...
            .map((p) =>
                destroyImage(p.photoPublicId as string).catch(() => {}),
            ),
        ...
        db
            .collection("messages")
            .deleteMany({ $or: [{ senderId: oid }, { recipientId: oid }] }),
        ...
        db
            .collection("reports")
            .deleteMany({ $or: [{ reporterId: oid }, { reportedId: oid }] }),
        db
            .collection("blocks")
            .deleteMany({ $or: [{ blockerId: oid }, { blockedId: oid }] }),
        db
            .collection("pokes")
            .deleteMany({ $or: [{ fromId: oid }, { toId: oid }] }),
        db.collection("chatboxes").deleteMany({ createdBy: oid }),
        db.collection("chatboxMessages").deleteMany({
            $or: [
                { senderId: oid },
                ...(chatboxIds.length > 0
                    ? [{ chatboxId: { $in: chatboxIds } }]
                    : []),
            ],
        }),
...
export async function adminSetBulletinPostLimitAction(
    limitValue: string,
): Promise<ActionResult> {
    await requireAdmin();
    ...
    if (raw) {
        ...
        if (!Number.isFinite(n) || n < 0 || n > 1000)
            return {
                error: "Limit must be a whole number between 0 and 1000.",
            };
        limit = Math.floor(n);
    }
    await getDb()
...
export async function adminDeleteUserByIdAction(
    userId: string,
): Promise<ActionResult> {
    const admin = await requireAdmin();
    ...
    if (admin._id.toString() === oid.toString())
        return { error: "You cannot delete yourself." };
...
export async function createChatboxAction(
    ...
    const visibilityValue = String(
        formData.get("visibility") || "public",
    ) as ChatboxVisibility;
    if (!name) return { error: "Please give your chatbox a name." };
    if (name.length > 60)
        return { error: "Chatbox name must be 60 characters or fewer." };
    if (!["public", "friends"].includes(visibilityValue))
        return { error: "Invalid chatbox visibility." };
...
export async function deleteChatboxAction(
    chatboxId: string,
): Promise<ActionResult> {
    const user = await requireUser();
    const chatbox = await getChatboxById(chatboxId);
    if (!chatbox) return { error: "Chatbox not found." };
    if (
        chatbox.createdBy.toString() !== user._id.toString() &&
        user.username !== "genggengpro"
    )
        return { error: "Not allowed." };
    ...
    await getDb()
        .collection("chatboxMessages")
        .deleteMany({ chatboxId: chatbox._id });
    revalidatePath("/chatboxes");
    redirect("/chatboxes");
}
...
export async function reportBugAction(
    ...
    if (body.length > 2000)
        return { error: "Bug report must be 2,000 characters or fewer." };
...
export async function toggleBugReportDoneAction(
    reportId: string,
): Promise<ActionResult> {
    const user = await getCurrentUser();
    if (!user || user.username !== "genggengpro")
        return { error: "Not allowed." };
    const db = getDb();
